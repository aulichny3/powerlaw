...
use powerlaw::{dist::{self, Distribution}, stats};
...
fn test_pareto_mle_correctness() {
    ...
    assert!((alpha - 0.66205).abs() < 0.0001, "Pareto MLE alpha estimation failed validation. Expected ~0.66205, got {}", alpha);
}
...
fn test_exponential_mle_correctness() {
    ...
    assert!((lambda - 1.0).abs() < 1e-9, "Exponential MLE lambda estimation failed validation.");
}
